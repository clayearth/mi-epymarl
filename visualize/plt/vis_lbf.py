...
def read_file(smac_map, load_num):
    ...
    for alg_name in algs:
        ...
        for num in range(1, nums+1):
            ...
            result_data = json.load(f)['test_return_mean']
            result_data = [r['value'] for r in result_data]
...
def plt_term_mean(smac_map, load_num, files, i, j):
    ...
    if load_num==100:
        plt.xticks((range(0, len(coll[alg_name]) + 3, 25)), ("0", "0.25", "0.50", "0.75", "1.00"))
...
if __name__ == '__main__':
    ...
    plt.legend(handles, labels, ncol=5, bbox_to_anchor=(0.5, 1.3))
    plt.savefig('./overview_results_lbf.pdf',bbox_inches='tight')
    # plt.show()